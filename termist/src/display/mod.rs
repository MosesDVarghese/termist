...
use termist_terminal::event::{EventListener, OnResize, WindowSize};
use termist_terminal::grid::Dimensions as TermDimensions;
use termist_terminal::index::{Column, Direction, Line, Point};
use termist_terminal::selection::Selection;
use termist_terminal::term::cell::Flags;
use termist_terminal::term::{
    self, point_to_viewport, LineDamageBounds, Term, TermDamage, TermMode, MIN_COLUMNS,
    MIN_SCREEN_LINES,
};
use termist_terminal::vte::ansi::{CursorShape, NamedColor};
...
impl Display {
    ...
    pub fn draw<T: EventListener>(
        ...
        // Add damage from termist's UI elements overlapping terminal.
        if self.collect_damage() {
        ...
        if matches!(self.raw_window_handle, RawWindowHandle::Xcb(_) | RawWindowHandle::Xlib(_)) {
            ...
            // will block to synchronize (this is `glClear` in Termist), which causes a
