...
use termist_terminal::event::{Event as TerminalEvent, EventListener, Notify};
use termist_terminal::event_loop::Notifier;
use termist_terminal::grid::{BidirectionalIterator, Dimensions, Scroll};
use termist_terminal::index::{Boundary, Column, Direction, Line, Point, Side};
use termist_terminal::selection::{Selection, SelectionType};
use termist_terminal::term::search::{Match, RegexSearch};
use termist_terminal::term::{self, ClipboardType, Term, TermMode};
...
impl<'a, N: Notify + 'a, T: EventListener> input::ActionContext<T> for ActionContext<'a, N, T> {
    ...
    fn spawn_new_instance(&mut self) {
        ...
        let termist = env_args.next().unwrap();

        let mut args: Vec<String> = Vec::new();

        // Reuse the arguments passed to Termist for the new instance.
        ...
        self.spawn_daemon(&termist, &args);
    }
