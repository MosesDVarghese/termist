...
pub fn deserialize_config(path: &Path, warn_pruned: bool) -> Result<Value> {
    ...
    if (extension == "yaml" || extension == "yml") && !contents.trim().is_empty() {
        warn!(
            "YAML config {path:?} is deprecated, please migrate to TOML using `termist migrate`"
        );
...
// TODO: Merge back with `load_imports` once `termist migrate` is dropped.
//
...
///
/// 1. $XDG_CONFIG_HOME/termist/termist.toml
/// 2. $XDG_CONFIG_HOME/termist.toml
/// 3. $HOME/.config/termist/termist.toml
/// 4. $HOME/.termist.toml
#[cfg(not(windows))]
pub fn installed_config(suffix: &str) -> Option<PathBuf> {
    let file_name = format!("termist.{suffix}");

    // Try using XDG location by default.
    xdg::BaseDirectories::with_prefix("termist")
        .ok()
        ...
        .or_else(|| {
            if let Ok(home) = env::var("HOME") {
                // Fallback path: $HOME/.config/termist/termist.toml.
                let fallback = PathBuf::from(&home).join(".config/termist").join(&file_name);
                if fallback.exists() {
                    return Some(fallback);
                }
                // Fallback path: $HOME/.termist.toml.
...
pub fn installed_config(suffix: &str) -> Option<PathBuf> {
    let file_name = format!("termist.{suffix}");
    dirs::config_dir().map(|path| path.join("termist").join(file_name)).filter(|new| new.exists())
}
