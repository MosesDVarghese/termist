...
use termist_terminal::event::Event as TerminalEvent;
use termist_terminal::event_loop::{EventLoop as PtyEventLoop, Msg, Notifier};
use termist_terminal::grid::{Dimensions, Scroll};
use termist_terminal::index::Direction;
use termist_terminal::sync::FairMutex;
use termist_terminal::term::test::TermSize;
use termist_terminal::term::{Term, TermMode};
use termist_terminal::tty;
...
/// Event context for one individual Termist window.
...
impl WindowContext {
    ...
    fn new(
        ...
        // Create context for the Termist window.
        Ok(WindowContext {
            preserve_title,
            terminal,
            display,
    ...
    pub fn draw(&mut self, scheduler: &mut Scheduler) {
        ...
        if !self.display.visual_bell.completed() {
            // We can get an OS redraw which bypasses termist's frame throttling, thus
