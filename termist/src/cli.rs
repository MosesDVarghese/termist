...
use termist_config::SerdeReplace;
use clap::{ArgAction, Args, Parser, Subcommand, ValueHint};
use log::{error, LevelFilter};
use serde::{Deserialize, Serialize};
use toml::Value;

use termist_terminal::tty::Options as PtyOptions;
...
/// CLI options for the main Termist executable.
...
pub struct Options {
    ...
    /// X11 window ID to embed Termist within (decimal or hexadecimal with "0x" prefix).
    #[clap(long)]
    pub embed: Option<String>,

    /// Specify alternative configuration file [default:
    /// $XDG_CONFIG_HOME/termist/termist.toml].
    #[cfg(not(any(target_os = "macos", windows)))]
    #[clap(long, value_hint = ValueHint::FilePath)]
    pub config_file: Option<PathBuf>,

    /// Specify alternative configuration file [default: %APPDATA%\termist\termist.toml].
    #[cfg(windows)]
    #[clap(long, value_hint = ValueHint::FilePath)]
    pub config_file: Option<PathBuf>,

    /// Specify alternative configuration file [default: $HOME/.config/termist/termist.toml].
    #[cfg(target_os = "macos")]
    #[clap(long, value_hint = ValueHint::FilePath)]
    pub config_file: Option<PathBuf>,
...
pub struct WindowIdentity {
    /// Defines the window title [default: Termist].
    #[clap(short = 'T', short_alias('t'), long)]
    pub title: Option<String>,

    /// Defines window class/app_id on X11/Wayland [default: Termist].
...
/// Send a message to the Termist socket.
#[cfg(unix)]
#[derive(Args, Debug)]
...
pub enum SocketMessage {
    /// Create a new window in the same Termist process.
    CreateWindow(WindowOptions),

    /// Update the Termist configuration.
    Config(IpcConfig),
}
...
mod tests {
    ...
    fn completions() {
        ...
        for (shell, file) in &[
            (Shell::Bash, "termist.bash"),
            (Shell::Fish, "termist.fish"),
            (Shell::Zsh, "_termist"),
        ] {
            let mut generated = Vec::new();
            clap_complete::generate(*shell, &mut clap, "termist", &mut generated);
        ...
        //
        // let mut file = File::create("../extra/completions/termist.bash").unwrap();
        // clap_complete::generate(Shell::Bash, &mut clap, "termist", &mut file);
        // let mut file = File::create("../extra/completions/termist.fish").unwrap();
        // clap_complete::generate(Shell::Fish, &mut clap, "termist", &mut file);
        // let mut file = File::create("../extra/completions/_termist").unwrap();
        // clap_complete::generate(Shell::Zsh, &mut clap, "termist", &mut file);
    }
}
